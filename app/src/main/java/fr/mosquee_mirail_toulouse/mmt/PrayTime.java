...
/**
 * Created by moham on 21/11/2017.
 */
...
public class PrayTime {
    ...
    private static double LATITUDE_TOULOUSE = 43.6042600;
    private static double LONGITUDE_TOULOUSE = 1.4436700;
    private static int TIMEZONE_TOULOUSE = 1;
    private static double ADJUST_MAGHREB_MMT = 5;
    ...
    public static ArrayList<String> DisplayPrayer(Calendar cal) {
        ...
        prayers.setMaghribMinutes(ADJUST_MAGHREB_MMT);

        int ofset = 60; //revoir 0 normalement ici 60 pour gestion heure été
        int[] offsets = {ofset, ofset, ofset, ofset, ofset, ofset, ofset}; // {Fajr,Sunrise,Dhuhr,Asr,Sunset,Maghrib,Isha}
        prayers.tune(offsets);

        ArrayList<String> prayerTimes = prayers.getPrayerTimes(cal,
                LATITUDE_TOULOUSE, LONGITUDE_TOULOUSE, TIMEZONE_TOULOUSE);
        return prayerTimes;
    }

    public static long getTomorowFajrTime(){

        Date date = new Date();     //date de ce jour
        Calendar cal = Calendar.getInstance();  //mise en forme
        cal.setTime(date);
        cal.add(Calendar.DAY_OF_WEEK,1);    //jour suivant

        String fajrTom = DisplayPrayer(cal).get(0);  //calculer fajr de demain
        int hour = Integer.parseInt(fajrTom.substring(0,2));
        String str = fajrTom.substring(3,5);
        ...
        long timeTomorowFajr = date.getTime();
        return timeTomorowFajr;
    }
